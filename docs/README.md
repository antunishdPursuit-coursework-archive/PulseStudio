...
| [hosted-schema.sql](./hosted-schema.sql) | The Postgres schema a sold, hosted copy would use for sign-in. Nothing runs it and nothing opens it; it moved out of `app/shared/auth/` on 2026-08-23 for the same reason the last two rows in this table moved — `app/` publishes everything under it |
| [the-server.md](./the-server.md) | The one Node process behind `npm start`: serves `app/`, answers `/api/chat` for Product C without the key ever reaching a browser, and guards the staff door — the passphrase, the signed session cookie, and why a static host can never open it |
| [TAKING-YOUR-FOLDER.md](./TAKING-YOUR-FOLDER.md) | How Kerrian, Manny and Dennis take their folder off `do-not-merge/v0.0.2` onto today's `main` — which branch to use, the three commits still waiting for their other half, why `app/shared/` must NOT come with the folder any more, and the one live defect this found |
