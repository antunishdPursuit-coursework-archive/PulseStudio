...
| `figures.ts` | The studio floor: one figure lifting, one riding, one running the length of the page. Every joint is a nested `<g>` that rotates about its own pivot — hip, then knee inside the hip's frame — so two straight lines become a leg that walks, with no path morphing and no runtime. Hand-rolled on purpose: a Lottie or Rive player is tens of kilobytes and, on a CDN, would make the privacy policy's "no third-party host" line false | A page declares `data-figure="lift\|cycle\|run"` and `data-figure-lane`; `mountFigures()` fills them and RETURNS any name it did not recognise, so an empty box is reported rather than shrugged at |
