...
const publishedSessionProblem=(candidate: unknown): string=>{if(!candidate||typeof candidate!=='object'||Array.isArray(candidate))return 'record must be an object';const session=candidate as Record<string, unknown>;for(const field of ['id','type','level','startsAt','time','room','instructor'])if(typeof session[field]!=='string')return `${field} must be a string`;if(!/^local-\d+$/.test(session['id'] as string))return 'id must be a local session id';if(Number.isNaN(Date.parse(session['startsAt'] as string)))return 'startsAt must be a datetime';if(!Number.isInteger(session['capacity'])||(session['capacity'] as number)<1)return 'capacity must be a positive integer';if(!Array.isArray(session['roster'])||(session['roster'] as unknown[]).length!==0)return 'roster must be an empty array';return '';};
